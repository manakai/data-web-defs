...
my $spec_path = path (__FILE__)->parent->parent->child
    ('local/www.whatwg.org/specs/web-apps/current-work/multipage/tokenization.html');
...
while (@node) {
  ...
  if ($node->node_type == $node->ELEMENT_NODE) {
    ...
    if ($ln =~ /^h[1-6]$/) {
      if ($node->text_content =~ /^[0-9.]+\s+(.+state)\s*$/) {
        $state_name = _n $1;
        ...
        for my $n (@{$node->children}) {
          ...
          if ($ln eq 'dt') {
            ...
            while (1) {
              if ($action =~ s/^Parse error\.\s*// or
              ...
              } elsif ($action =~ s/^(?:S|s|Finally, s)witch to the ([A-Za-z0-9 ._()-]+ state)(?:\.\s*|\s*$)//) {
                ...
                push @action, {type => 'switch-by-temp',
                       ...
                       $action =~ s/^Emit (?:the current|that|the) (?:tag|DOCTYPE|comment) token\.\s*//) {
                push @action, {type => 'emit'};
              } elsif ($action =~ s/^Emit the current input character as a character token\.\s*//) {
                ...
                push @action, {type => 'append', field => $2,
                               value => chr hex $1};
              } elsif ($action =~ s/^Append two U\+002D HYPHEN-MINUS characters \(-\),?(?: and|) ([^.]+ to the comment token's data\.)\s*/Append $1/) {
                ...
                push @action, {type => 'append', field => $2,
                               value => chr hex $1};
              } elsif ($action =~ s/^Append the current input character to the current attribute's (name|value)\.\s*//) {
                push @action, {type => 'append-attr', field => $1};
                ...
                push @action, {type => 'append-temp', offset => 0x0020};
              } elsif ($action =~ s/^Set the temporary buffer to the empty string\.\s*//) {
                push @action, {type => 'set-temp-empty'};
              } elsif ($action =~ s/^Create a new ((?:start tag|end tag|DOCTYPE) token)(?:\.\s*|, and |, )//) {
              ...
              } elsif ($action =~ s/^Set that attribute's name to the current input character, and its value to the empty string\.\s*//) {
                push @action, {type => 'set-attr', field => 'name'};
                push @action, {type => 'set-empty-attr', field => 'value'};
