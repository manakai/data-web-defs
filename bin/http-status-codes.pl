...
for ((map { [$_, 'HTTP'] } @{(parse 'iana-http-statuses.xml')->query_selector_all ('record')}),
     (map { [$_, 'RTSP'] } @{(parse 'iana-rtsp-statuses.xml')->query_selector_all ('registry[id="rtsp-parameters-3"] record')}),
     (map { [$_, 'SIP'] } @{(parse 'iana-sip-statuses.xml')->query_selector_all ('registry[id="sip-parameters-7"] record')})) {
  my ($el, $proto) = @$_;
